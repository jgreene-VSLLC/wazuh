    ...
    def get_agents_overview(status="all", os_platform="all", os_version="all", manager_host="all",
        ...
        if status != "all":
            ...
            for status in list_status:
                status = status.lower()
                if status == 'active':
                    query += '((last_keepalive >= :time_active or id = 0) '
                elif status == 'disconnected':
                    query += '(last_keepalive < :time_active '
                elif status == "never connected" or status == "neverconnected":
                    query += '(last_keepalive IS NULL AND id != 0 '
                elif status == 'pending':
                    query += '(last_keepalive IS NOT NULL AND version IS NULL '
                else:
                    raise WazuhException(1729, status)

                if timeframe != 'all':
                    if status == "never connected" or status == "neverconnected":
                        query += " AND CAST(strftime('%s', date_Add) AS INTEGER) < CAST(strftime('%s', 'now', 'localtime') AS INTEGER) - :timeframe"
                    else:
                        query += " AND CAST(strftime('%s', last_keepalive) AS INTEGER) < CAST(strftime('%s', 'now', 'localtime') AS INTEGER) - :timeframe"
